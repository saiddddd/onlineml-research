...
class ModelEvaluator(abc.ABC):
    ...
    @abc.abstractmethod
    def predict_proba_true_class_by_month_year(self, i_month_year=None):
        pass

    def get_testing_x_y_full_set(self):

        full_testing_dataset = self._data_loader_for_testing.get_full_df()
        full_testing_dataset = full_testing_dataset.replace([np.inf, -np.inf], np.nan)
        full_testing_dataset.dropna(inplace=True)

        X_test = copy.deepcopy(full_testing_dataset)
        y_test = X_test.pop(self._label)
        X_test.drop([self._data_loader_for_testing.get_time_series_column_name()], axis=1, inplace=True)
    ...
    def get_testing_x_y_monthly_subset(self, i_month_year):

        sub_df_by_month_year = self._data_loader_for_testing.get_sub_df_by_month_year(i_month_year)
        sub_df_by_month_year = sub_df_by_month_year.replace([np.inf, -np.inf], np.nan)
        sub_df_by_month_year.dropna(inplace=True)

        X_test = copy.deepcopy(sub_df_by_month_year)
        y_test = X_test.pop(self._label)
        X_test.drop([self._data_loader_for_testing.get_time_series_column_name()], axis=1, inplace=True)
...
class SklearnModelEvaluator(ModelEvaluator):
    ...
    def predict_proba_true_class_by_month_year(self, i_month_year=None):
        ...
        if i_month_year is None:
            X_test, y_test = self.get_testing_x_y_full_set()
        else:
            X_test, y_test = self.get_testing_x_y_monthly_subset(i_month_year)
...
class RiverModelEvaluator(ModelEvaluator):
    ...
    def _run_prediction(self, X_test, y_test, do_online_training):
        ...
        for index, raw in tqdm(X_test.iterrows(), total=X_test.shape[0]):
            try:
                ...
                if isinstance(pred_proba_result, dict):
                    ...
                    print(
                        'Critical error: prediction proba result from riverml Model is not dict type (unexpect type found !) ')
    ...
    def predict_proba_true_class_by_date(self, i_date=None, do_online_training=False):
        ...
        pred_proba_result_list, corresponding_ans = self._run_prediction(X_test, y_test, do_online_training)

        return pred_proba_result_list, corresponding_ans

    def predict_proba_true_class_by_month_year(self, i_month_year=None, do_online_training=False):
